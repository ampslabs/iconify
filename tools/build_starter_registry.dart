...
const githubRawBase = 'https://raw.githubusercontent.com/iconify/icon-sets/master';
...
const popularNames = [
  'home', 'account', 'settings', 'search', 'menu', 'close', 'check', 'chevron-right',
  'chevron-left', 'chevron-down', 'chevron-up', 'plus', 'minus', 'delete', 'edit',
  'calendar', 'clock', 'camera', 'image', 'video', 'music', 'heart', 'star', 'flag',
  'bell', 'mail', 'send', 'share', 'download', 'upload', 'link', 'lock', 'unlock',
  'eye', 'eye-off', 'map-marker', 'phone', 'smartphone', 'laptop', 'desktop', 'monitor',
  'wifi', 'bluetooth', 'battery', 'shopping-cart', 'credit-card', 'bank', 'wallet',
  'briefcase', 'database', 'cloud', 'cloud-upload', 'cloud-download', 'folder', 'file',
  'copy', 'clipboard', 'printer', 'trash', 'info', 'alert', 'help', 'question', 'error',
  'warning', 'success', 'arrow-right', 'arrow-left', 'arrow-up', 'arrow-down', 'refresh',
  'sync', 'cog', 'filter', 'sort', 'list', 'grid', 'table', 'play', 'pause', 'stop',
  'skip-next', 'skip-prev', 'repeat', 'shuffle', 'volume-up', 'volume-off', 'mic',
  'headset', 'cast', 'tv', 'radio', 'sun', 'moon', 'cloud-sun', 'cloud-moon', 'wind',
  'water', 'thermometer', 'gauge', 'speedometer', 'tools', 'hammer', 'wrench', 'pen',
  'brush', 'palette', 'color-helper', 'eye-dropper', 'format-text', 'bold', 'italic',
  'underline', 'align-left', 'align-center', 'align-right', 'align-justify', 'zoom-in',
  'zoom-out', 'maximize', 'minimize', 'fullscreen', 'exit-to-app', 'logout', 'login',
  'fingerprint', 'key', 'security', 'shield', 'fire', 'leaf', 'tree', 'flower', 'coffee',
  'food', 'pizza', 'car', 'bus', 'train', 'airplane', 'bike', 'walk', 'run', 'swim',
  'medkit', 'heart-pulse', 'hospital', 'doctor', 'shield-check', 'verified',
];
...
Future<void> main() async {
  ...
  try {
    ...
    final collectionsResponse = await client.get(Uri.parse('$githubRawBase/collections.json'));
    if (collectionsResponse.statusCode != 200) {
      throw Exception('Failed to fetch collections.json: ${collectionsResponse.statusCode}');
    }
    
    final Map<String, dynamic> allCollections = jsonDecode(collectionsResponse.body);
    ...
    for (final entry in starterCollections.entries) {
      ...
      final response = await client.get(Uri.parse('$githubRawBase/json/$prefix.json'));
      ...
      final Map<String, dynamic> icons = fullData['icons'] as Map<String, dynamic>;
      ...
      final Map<String, dynamic>? aliases = fullData['aliases'] as Map<String, dynamic>?;
      if (aliases != null) {
      ...
      print('  ✅ Saved $prefix.json (${trimmedIcons.length} icons, ${trimmedAliases.length} aliases, ${(encoded.length / 1024).toStringAsFixed(1)} KB)');
    }
