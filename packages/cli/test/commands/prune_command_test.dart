...
''');
      ...
      await File(p.join(tempDir.path, 'assets', 'iconify', 'used_icons.json')).writeAsString(jsonEncode({
        'schemaVersion': 1,
        'icons': {
          'mdi:home': {'body': '<path/>'},
    ...
    test('prunes stale icons with confirmation', () async {
      ...
      await File(p.join(tempDir.path, 'lib', 'main.dart')).writeAsString("IconifyIcon('mdi:home')");
    ...
    test('respects --force flag', () async {
      ...
      expect(result, equals(ExitCode.success.code));
      verifyNever(() => logger.confirm(any(), defaultValue: any(named: 'defaultValue')));
    ...
    test('respects --dry-run flag', () async {
      ...
      final cacheFile = File(p.join(tempDir.path, 'assets', 'iconify', 'used_icons.json'));
      final cacheJson = jsonDecode(await cacheFile.readAsString()) as Map<String, dynamic>;
      expect((cacheJson['icons'] as Map<String, dynamic>).length, equals(2)); // No icons removed
    });

    test('completes with message if nothing to prune', () async {
      await File(p.join(tempDir.path, 'lib', 'main.dart')).writeAsString("IconifyIcon('mdi:home'), IconifyIcon('mdi:account')");

      final result = await runner.run(['prune']);

      expect(result, equals(ExitCode.success.code));
      verify(() => progress.complete(any(that: contains('No stale icons found')))).called(1);
    });
  });
}
